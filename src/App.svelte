...
<main class="min-h-screen bg-[#f7f4ec] px-6 py-8 text-stone-950">
  <section class="mx-auto flex max-w-6xl flex-col gap-8">
    <header class="rounded-[2rem] border border-stone-300/70 bg-[#fffaf0] p-8 shadow-sm">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
        ...
        <div class="flex flex-wrap gap-2">
          <button class="px-3 py-1 text-xs font-medium rounded-full transition-colors {viewState === 'loading' ? 'bg-stone-900 text-white' : 'bg-stone-200 text-stone-700 hover:bg-stone-300'}" onclick={() => viewState = 'loading'}>Loading</button>
          <button class="px-3 py-1 text-xs font-medium rounded-full transition-colors {viewState === 'error' ? 'bg-stone-900 text-white' : 'bg-stone-200 text-stone-700 hover:bg-stone-300'}" onclick={() => viewState = 'error'}>Error</button>
          <button class="px-3 py-1 text-xs font-medium rounded-full transition-colors {viewState === 'empty' ? 'bg-stone-900 text-white' : 'bg-stone-200 text-stone-700 hover:bg-stone-300'}" onclick={() => viewState = 'empty'}>Empty</button>
          <button class="px-3 py-1 text-xs font-medium rounded-full transition-colors {viewState === 'success' ? 'bg-stone-900 text-white' : 'bg-stone-200 text-stone-700 hover:bg-stone-300'}" onclick={() => viewState = 'success'}>Success</button>
        </div>
      </div>
      <div class="grid gap-6 lg:grid-cols-[1.4fr_0.8fr] lg:items-end">
        <div>
          <h1 class="max-w-3xl text-5xl font-black tracking-[-0.05em] text-stone-950 md:text-7xl">
    ...
    {#if viewState === 'loading'}
      <div class="flex flex-col items-center justify-center p-16 rounded-[2rem] border border-stone-300/80 bg-white/70 shadow-sm animate-pulse">
        <p class="text-xl font-bold text-stone-500">Loading symphony runtime data...</p>
      </div>
    {:else if viewState === 'error'}
      <div class="flex flex-col items-center justify-center p-16 rounded-[2rem] border border-rose-300/80 bg-rose-50 shadow-sm">
        ...
        <button class="mt-6 px-5 py-2.5 bg-rose-600 text-white font-medium rounded-xl hover:bg-rose-700 transition-colors shadow-sm" onclick={() => viewState = 'loading'}>Retry Connection</button>
      </div>
    {:else if viewState === 'empty'}
      <div class="flex flex-col items-center justify-center p-16 rounded-[2rem] border border-stone-300/80 bg-white/70 shadow-sm">
